...
print("\n" + "=" * 50)
print(f"UberEats 訂單抓取程式 (肯德基 KFC) v{VERSION} (更新日期: {LAST_UPDATED})")
print("=" * 50)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    if os.path.exists("ubereats_kfc_cookies.pkl"):
        try:
            # os.remove("ubereats_kfc_cookies.pkl")
    ...
    print("   如需強制手動登入，請使用參數執行: python ubereats_kfc.py login")
...
# 以下兩行被註釋掉
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome\UserData_ubereats_kfc')
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("ubereats_kfc_cookies.pkl"):
        print("⚠️ Cookie 文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie 文件不存在")
    
    try:
        # 載入保存的 Cookie
        with open("ubereats_kfc_cookies.pkl", "rb") as file:
    ...
    if os.path.exists("ubereats_kfc_cookies.pkl") and not force_login:
        try:
            # os.remove("ubereats_kfc_cookies.pkl")
    ...
    if login_detected or (msvcrt.kbhit() and msvcrt.getch().decode('utf-8').lower() == 'y'):
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("ubereats_kfc_cookies.pkl", "wb") as file:
...
# 🔹 6️⃣ **點擊肯德基KFC漢堡 紙包雞飯專賣 北投光明店**
try:
    # 使用 data-testid 屬性定位肯德基餐廳選項
    kfc_restaurant = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "li[data-testid='menu-item-肯德基KFC漢堡 紙包雞飯專賣 北投光明店']")))
    kfc_restaurant.click()
    print("✅ 成功點擊『肯德基KFC漢堡 紙包雞飯專賣 北投光明店』！")
except Exception as e:
    print(f"❌ 找不到『肯德基KFC漢堡 紙包雞飯專賣 北投光明店』，錯誤：{e}")
    # 嘗試使用 XPath 替代方法
    try:
        kfc_xpath = "//div[contains(text(), '肯德基KFC漢堡 紙包雞飯專賣 北投光明店')]"
        kfc_element = wait.until(EC.element_to_be_clickable((By.XPATH, kfc_xpath)))
        kfc_element.click()
        print("✅ 使用 XPath 成功點擊『肯德基KFC漢堡 紙包雞飯專賣 北投光明店』！")
        ...
        try:
            ...
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].textContent.includes('肯德基KFC') && elements[i].textContent.includes('北投光明店')) {
                    elements[i].click();
                    return true;
                }
            }
            ...
            if result:
                print("✅ 使用 JavaScript 成功點擊『肯德基KFC漢堡 紙包雞飯專賣 北投光明店』！")
            else:
                print("❌ 無法找到『肯德基KFC漢堡 紙包雞飯專賣 北投光明店』。")
                # 嘗試更廣泛的匹配
                js_script_broader = """
                var elements = document.querySelectorAll('li');
                for (var i = 0; i < elements.length; i++) {
                    if (elements[i].textContent.includes('肯德基') || elements[i].textContent.includes('KFC')) {
                        elements[i].click();
                        return true;
                    }
                }
                return false;
                """
                result = driver.execute_script(js_script_broader)
                if result:
                    print("✅ 使用廣泛匹配成功點擊肯德基相關餐廳！")
                else:
                    print("❌ 無法找到任何肯德基相關餐廳，終止程序。")
                    driver.quit()
                    exit()
        except Exception as js_error:
            print(f"❌ JavaScript 點擊失敗，錯誤：{js_error}")
...
try:
        ...
        itemNameElements.forEach((nameElem, index) => {
            try {
                ...
                if (priceContainer) {
                    ...
                    // 3. 解析KFC的食物細項 - 專門針對飲料選擇和加購項目
                    const foodDetails = {};

                    // 使用更準確的選擇器來找到食物細項的標題和內容
                    try {
                        // 尋找所有可能的食物細項選項容器
                        // 首先找到包含商品詳情的整個容器
                        const orderItemContainer = priceContainer;
                        
                        // 尋找所有標題元素 - 這些通常包含"飲料選擇"、"加購"等字樣
                        // KFC的飲料選擇通常有固定格式: 飲料選擇(1/2) | Choose Beverage(1/2)
                        const optionTitles = [];
                        
                        // 使用文本內容查找標題元素，避免依賴class
                        const allDivs = orderItemContainer.querySelectorAll('div');
                        allDivs.forEach(div => {
                            const text = div.textContent.trim();
                            // 檢查是否包含飲料選擇或加購關鍵字
                            if ((text.includes('飲料選擇') || text.includes('飲品選擇') || 
                                 text.includes('Choose Beverage') || text.includes('加購') || 
                                 text.includes('Add-Ons')) && 
                                text.length < 100) { // 避免抓取過長的文本
                                
                                // 去除標題中的價格信息
                                let cleanTitle = text;
                                if (text.includes('|')) {
                                    cleanTitle = text.split('|')[0].trim();
                                }
                        ...
                        optionTitles.forEach(title => {
                            // 尋找跟在標題後面的列表項目
                            let nextElement = title.element.nextElementSibling;
                                ...
                                if (listItems.length > 0) {
                                    ...
                                    listItems.forEach(item => {
                                        ...
                                        if (itemText && itemText.length > 0) {
                                            items.push(itemText);
                                            console.log(`- 找到項目: "${itemText}" (屬於 "${title.text}")`);
                                        }
                                    });
                                    
                                    // 只有當找到項目時才添加到食物細項
                                    if (items.length > 0) {
                                        foodDetails[title.text] = items.join(', ');
                                    }
                                }
                            } else {
                                // 在更複雜的布局中搜索項目
                                // 查找從此標題元素到下一個標題元素之間的所有可能的項目元素
                                let currentElem = title.element.nextElementSibling;
                                const items = [];
                                
                                while (currentElem) {
                                    // 檢查是否是下一個標題開始
                                    const currentText = currentElem.textContent.trim();
                                    if ((currentText.includes('飲料選擇') || 
                                         currentText.includes('飲品選擇') || 
                                         currentText.includes('Choose Beverage') || 
                                         currentText.includes('加購') || 
                                         currentText.includes('Add-Ons')) && 
                                    ...
                                    if (currentElem.tagName === 'LI' || 
                                       (currentElem.tagName === 'DIV' && 
                                        !currentText.includes('飲料選擇') && 
                                        !currentText.includes('加購') &&
                                        ...
                                        console.log(`- 找到項目: "${currentText}" (屬於 "${title.text}")`);
                                    }
                        ...
                        // 如果未找到任何項目，嘗試直接搜索列表項目並根據上下文關聯
                        if (Object.keys(foodDetails).length === 0) {
                            const allListItems = orderItemContainer.querySelectorAll('li');
                            
                            if (allListItems.length > 0) {
                                console.log("使用備用方法查找食物細項...");
                                ...
                                allListItems.forEach(item => {
                                    ...
                                    for (let i = 0; i < 5; i++) {
                                        ...
                                        // 檢查是否是標題
                                        if ((prevText.includes('飲料選擇') || 
                                             prevText.includes('飲品選擇') || 
                                             prevText.includes('Choose Beverage') || 
                                             prevText.includes('加購') || 
                                             prevText.includes('Add-Ons')) && 
                                            prevText.length < 100) {
                                            
                                            // 清理標題文本
                                            let category = prevText;
                                            ...
                                            categoryFound = true;
                                            console.log(`- 備用方法找到項目: "${itemText}" (屬於 "${category}")`);
                                            break;
                                        }
                                    ...
                                    // 如果找不到類別，檢查是否包含飲料關鍵字
                                    if (!categoryFound) {
                                        if (itemText.includes('茶') || 
                                            itemText.includes('可樂') || 
                                            itemText.includes('汽水') || 
                                            itemText.includes('Tea') || 
                                            itemText.includes('Cola') || 
                                            itemText.includes('7.Up') || 
                                            itemText.includes('雪碧')) {
                                            ...
                                            categorizedItems[category].push(itemText);
                                            console.log(`- 通過關鍵字識別為飲料: "${itemText}"`);
                                        } else if (itemText.includes('加點') || 
                                                  itemText.includes('蛋撻') || 
                                                  itemText.includes('甜點')) {
                                            ...
                                            categorizedItems[category].push(itemText);
                                            console.log(`- 通過關鍵字識別為加購: "${itemText}"`);
                                        }
                                    }
                                });
                                
                                // 將分類結果轉換為foodDetails格式
                                for (const [category, items] of Object.entries(categorizedItems)) {
                                    if (items.length > 0) {
                                        foodDetails[category] = items.join(', ');
                                    }
                                }
                            }
                        }
                    ...
                    for (const [category, itemsText] of Object.entries(foodDetails)) {
                        foodDetailsList.push(`${category}：${itemsText}`);
                    }
                    
                    console.log(`最終解析的食物細項:`, foodDetails);
    ...
    if order_items and 'items' in order_items and len(order_items['items']) > 0:
        ...
        with open("ubereats_kfc.json", "w", encoding="utf-8") as f:
            json.dump(output_json, f, ensure_ascii=False, indent=4)
        print("\n✅ 訂單信息已保存到 'ubereats_kfc.json'")
    else:
        print("❌ 未能抓取到訂單信息")
