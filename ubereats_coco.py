...
print("\n" + "=" * 50)
print(f"UberEats 訂單抓取程式 (CoCo都可 北投中央北店) v{VERSION} (更新日期: {LAST_UPDATED})")
print("=" * 50)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    if os.path.exists("ubereats_coco_cookies.pkl"):
        try:
            # os.remove("ubereats_coco_cookies.pkl")
    ...
    print("   如需強制手動登入，請使用參數執行: python ubereats_coco.py login")
...
# 以下兩行被註釋掉
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome\UserData_ubereats_coco')
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("ubereats_coco_cookies.pkl"):
        print("⚠️ Cookie 文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie 文件不存在")
    
    try:
        # 載入保存的 Cookie
        with open("ubereats_coco_cookies.pkl", "rb") as file:
    ...
    if os.path.exists("ubereats_coco_cookies.pkl") and not force_login:
        try:
            # os.remove("ubereats_coco_cookies.pkl")
    ...
    if login_detected or (msvcrt.kbhit() and msvcrt.getch().decode('utf-8').lower() == 'y'):
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("ubereats_coco_cookies.pkl", "wb") as file:
...
# 🔹 6️⃣ **點擊CoCo都可 北投中央北店**
try:
    # 使用 data-testid 屬性定位CoCo都可餐廳選項
    milktea_restaurant = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "li[data-testid='menu-item-CoCo都可 北投中央北店']")))
    milktea_restaurant.click()
    print("✅ 成功點擊『CoCo都可 北投中央北店』！")
except Exception as e:
    print(f"❌ 找不到『CoCo都可 北投中央北店』，錯誤：{e}")
    # 嘗試使用 XPath 替代方法
    try:
        milktea_xpath = "//div[contains(text(), 'CoCo都可 北投中央北店')]"
        milktea_element = wait.until(EC.element_to_be_clickable((By.XPATH, milktea_xpath)))
        milktea_element.click()
        print("✅ 使用 XPath 成功點擊『CoCo都可 北投中央北店』！")
        ...
        try:
            ...
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].textContent.includes('CoCo都可 北投中央北店')) {
                    elements[i].click();
                    return true;
                }
            }
            ...
            if result:
                print("✅ 使用 JavaScript 成功點擊『CoCo都可 北投中央北店』！")
            else:
                print("❌ 無法找到『CoCo都可 北投中央北店』。")
                
                # 嘗試更廣泛的搜索，只查找包含"CoCo都可"的元素
                js_script_broader = """
                var elements = document.querySelectorAll('li[role="menuitem"]');
                for (var i = 0; i < elements.length; i++) {
                    if (elements[i].textContent.includes('CoCo都可')) {
                        console.log('找到CoCo都可相關餐廳：' + elements[i].textContent);
                        elements[i].click();
                        return true;
                    }
                }
                
                // 如果還找不到，嘗試查找任何可能包含"CoCo都可"的元素
                var allElements = document.querySelectorAll('div, li, button, span');
                for (var i = 0; i < allElements.length; i++) {
                    if (allElements[i].textContent.includes('CoCo都可')) {
                        console.log('找到CoCo都可元素：' + allElements[i].textContent);
                        // 尋找可點擊的父元素
                ...
                if broader_result:
                    print("✅ 使用更廣泛的搜索成功點擊包含『CoCo都可』的元素！")
                else:
                    print("❌ 即使使用更廣泛的搜索，也無法找到『CoCo都可』相關元素")
                driver.quit()
                exit()
        except Exception as js_error:
            print(f"❌ JavaScript 點擊失敗，錯誤：{js_error}")
...
try:
    ...
    order_items = driver.execute_script("""
        ...
        detailContainers.forEach((container, index) => {
            try {
                ...
                // 3. 提取食物細項 - 使用CoCo都可的HTML結構特點
                const foodDetails = {};
                
                // 找到所有标签和对应的值
                // 直接基于文本内容查找标签div，而不依赖可能变化的class
                // 使用遍历的方式来找到所有细项标签div和对应的ul>li
                // 注意CoCo都可菜单的特点是每个选项有"xxx Level"的说明，如"甜度 Sweetness Level"
                const divElements = container.querySelectorAll('div');
                
                // 定义我们要寻找的标签关键词
                const targetLabels = ['甜度', '冰量', '加料', '飲品溫度', '溫度'];
                
                // 遍历所有div找到可能是标签的元素
                divElements.forEach(div => {
                    const divText = div.textContent.trim();
                    
                    // 检查这个div是否包含我们关注的标签关键词
                    ...
                    if (matchedLabel) {
                        console.log(`找到标签: ${divText}`);
                        
                        // 查找紧跟在标签之后的ul元素
                        let nextElement = div.nextElementSibling;
                        if (nextElement && nextElement.tagName === 'UL') {
                            // 获取第一个li元素的文本，通常包含选项值
                            const liElement = nextElement.querySelector('li');
                            if (liElement) {
                                const optionText = liElement.textContent.trim();
                                
                                // 保存到食物细项对象
                                foodDetails[matchedLabel] = optionText;
                                console.log(`提取到选项: ${matchedLabel} => ${optionText}`);
                            }
                        }
                    }
                });
                
                // 额外尝试查找更常见的标签格式，比如"甜度 Sweetness Level"这样的完整标签
                divElements.forEach(div => {
                    const divText = div.textContent.trim();
                    
                    // 使用特定的完整标签匹配
                    if (divText.includes('甜度 Sweetness Level')) {
                        // 如果已经有了甜度，就不再重复添加
                        if (!foodDetails['甜度']) {
                            let nextElement = div.nextElementSibling;
                            if (nextElement && nextElement.tagName === 'UL') {
                                const liElement = nextElement.querySelector('li');
                                if (liElement) {
                                    foodDetails['甜度'] = liElement.textContent.trim();
                                }
                            }
                        }
                    } else if (divText.includes('冰量 Ice Level')) {
                        if (!foodDetails['冰量']) {
                            let nextElement = div.nextElementSibling;
                            if (nextElement && nextElement.tagName === 'UL') {
                                const liElement = nextElement.querySelector('li');
                                if (liElement) {
                                    foodDetails['冰量'] = liElement.textContent.trim();
                                }
                            }
                        }
                    } else if (divText.includes('加料 Add On')) {
                        if (!foodDetails['加料']) {
                            let nextElement = div.nextElementSibling;
                            if (nextElement && nextElement.tagName === 'UL') {
                                const liElement = nextElement.querySelector('li');
                                if (liElement) {
                                    foodDetails['加料'] = liElement.textContent.trim();
                                }
                            }
                        }
                    } else if (divText.includes('飲品溫度 Beverage Temperature')) {
                        if (!foodDetails['飲品溫度']) {
                            let nextElement = div.nextElementSibling;
                            if (nextElement && nextElement.tagName === 'UL') {
                                const liElement = nextElement.querySelector('li');
                                if (liElement) {
                                    foodDetails['飲品溫度'] = liElement.textContent.trim();
                                }
                            }
                        }
                    }
        ...
        if (orderItems.length === 0) {
            ...
            nameElems.forEach((nameElem, index) => {
                try {
                    ...
                    // 使用特殊类选择器找到标签元素，注意CoCo都可的HTML结构有一些特殊性
                    ...
                    optionLabels.forEach(labelElem => {
                        ...
                        console.log(`处理标签: ${labelText}`);
                        
                        // 提取标签中的主要部分(冒号前面)
                        let mainLabel = labelText;
                        // 查找空格位置，通常是中英文分隔处
                        const spaceIndex = labelText.indexOf(' ');
                        if (spaceIndex > 0) {
                            mainLabel = labelText.substring(0, spaceIndex);
                        }
                        
                        // 查找下一个元素，应该是<ul>
                        const nextElement = labelElem.nextElementSibling;
                        if (nextElement && nextElement.tagName === 'UL') {
                            const liElement = nextElement.querySelector('li');
                            if (liElement) {
                                const valueText = liElement.textContent.trim();
                                foodDetails[mainLabel] = valueText;
                                console.log(`提取到选项: ${mainLabel} => ${valueText}`);
                            }
                        }
                    });
    ...
    if order_items and 'items' in order_items and len(order_items['items']) > 0:
        ...
        for item in order_items['items']:
            ...
            # 處理食物細項 - 從物件轉換為指定格式的數組
            food_details_array = []
            if isinstance(item["食物細項"], dict) and item["食物細項"]:
                # 將食物細項物件轉換為數組格式
                for key, value in item["食物細項"].items():
                    food_details_array.append(f"{key}：{value}")
            
            ordered_item["食物細項"] = food_details_array
        ...
        if order_items['總價'] != "總價未知":
            # 將JSON格式化為更容易閱讀的形式
            with open("ubereats_coco.json", "w", encoding="utf-8") as f:
                json.dump(output_json, f, ensure_ascii=False, indent=4)
            print("\n✅ 訂單信息已保存到 'ubereats_coco.json'")
        else:
            print("\n⚠️ 總價未知，不輸出JSON文件")
    else:
        print("❌ 未能抓取到訂單信息")
