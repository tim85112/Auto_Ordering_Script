...
# 移除無痕模式，改為使用其他設置減少磁盤空間使用
# options.add_argument("--incognito")
options.add_argument("--disable-application-cache")  # 禁用應用緩存
options.add_argument("--media-cache-size=1")  # 最小化媒體緩存
options.add_argument("--disk-cache-size=1")  # 最小化磁盤緩存
options.add_argument("--disable-gpu-shader-disk-cache")  # 禁用GPU著色器緩存

# ✅ 檢查是否需要首次手動登入
print("\n" + "="*60)
print("Foodpanda 迷客夏自動訂餐程序")
print("="*60)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    print("   如需強制手動登入，請使用參數執行: python foodpanda_milkshop.py login")

print("="*60 + "\n")

# 啟動 WebDriver
try:
    # 不使用webdriver_manager，直接初始化Chrome WebDriver
    driver = webdriver.Chrome(options=options)
    wait = WebDriverWait(driver, 30)  # 增加等待時間到30秒
except Exception as e:
    print(f"❌ 啟動Chrome失敗: {str(e)}")
    print("詳細錯誤信息:")
    import traceback
...
# 讀取訂單資料
order_file_path = "ubereats_milkshop.json"
...
# 📌 Foodpanda 迷客夏 URL
foodpanda_url = "https://www.foodpanda.com.tw/restaurant/pt0a/mi-ke-xia-milksha-tai-bei-bei-tou-dian"
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("foodpanda_milkshop_cookies.pkl"):
        print("⚠️ Cookie文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie文件不存在")
        
    # 讀取Cookie文件
    with open("foodpanda_milkshop_cookies.pkl", "rb") as file:
    ...
    if login_detected:
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("foodpanda_milkshop_cookies.pkl", "wb") as file:
...
for order in orders:
    try:
        ...
        food_options_list = order.get('食物細項', [])  # 迷客夏訂單格式是數組
        ...
        try:
            ...
            print(f"🔍 搜尋餐點名稱: {original_menu_item}")
            
            search_input = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "input[placeholder='店內搜尋']")))
            search_input.clear()
            search_input.send_keys(original_menu_item)
            time.sleep(2)
            print(f"✅ 已搜尋餐點: {original_menu_item}")
        except Exception as e:
            ...
            try:
                ...
                if search_inputs:
                    ...
                    search_inputs[0].send_keys(original_menu_item)
                    time.sleep(2)
                    print(f"✅ 已使用替代方法搜尋餐點: {original_menu_item}")
                else:
                    print("❌ 找不到任何輸入框")
                    continue
        ...
        try:
            ...
            print(f"❌ 無法點擊餐點 {original_menu_item}: {str(e)}")
            # 嘗試其他方式
            try:
                # 嘗試通過餐點名稱定位
                menu_elements = driver.find_elements(By.XPATH, f"//*[contains(text(), '{original_menu_item}')]")
                if menu_elements:
                    ...
                    print(f"❌ 找不到包含 '{original_menu_item}' 的元素")
                    continue
            except Exception as e2:
        ...
        for option in food_options_list:
            option = option.strip()
            # 已處理的尺寸選項
            if option == size_option:
                continue
            elif option in ["微糖", "半糖", "少糖", "正常糖", "無糖", "一分糖","二分糖", "三分糖"]:
                food_options_obj["甜度"] = option
            elif option in ["熱", "溫", "常溫", "正常冰", "少冰", "微冰", "去冰"]:
                food_options_obj["溫度"] = option
            else:
                # 其他選項視為加料
                food_options_obj["加料"] = option
        
        print(f"🔧 解析食物細項: {food_options_list}")
        print(f"🔧 結構化食物選項: {food_options_obj}")
        
        # 先處理杯子尺寸選擇（如果有）
        if size_option:
            try:
                # 嘗試找到杯子尺寸選項組，修改XPath以修復語法錯誤
                print(f"⏳ 嘗試選擇杯子尺寸: {size_option}")
                
                # 先檢查頁面上是否有尺寸/容量選項組
                size_groups = driver.find_elements(By.XPATH, 
                    "//div[contains(@class, 'modifier-group') and contains(.//div/text(), '容量') or contains(.//div/text(), '尺寸') or contains(.//div/text(), '大小')]")
                
                if not size_groups:
                    print(f"⚠️ 未找到尺寸/容量選項組，此飲料可能不需要選擇杯型，直接跳過")
                    # 如果找不到尺寸選項組，直接跳過
                    raise Exception("無需選擇杯型")
                    
                # 找到尺寸選項組後進行選擇
                size_group = size_groups[0]
                print(f"✅ 找到杯子尺寸/容量選項組")
                
                # 在尺寸組內找到特定尺寸的選項並點擊
                size_option_elem = size_group.find_element(By.XPATH, f".//label[.//span[contains(text(), '{size_option}')]]")
                driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", size_option_elem)
                time.sleep(1)
                driver.execute_script("arguments[0].click();", size_option_elem)
                print(f"✅ 已選擇杯子尺寸: {size_option}")
                time.sleep(1)
            except Exception as e:
                print(f"⚠️ 選擇杯子尺寸 {size_option} 時出錯: {str(e)}")
                # 嘗試通用方法
                try:
                    # 如果有尺寸選項但無法通過上述方法選擇，直接搜索包含尺寸文字的標籤
                    all_size_options = driver.find_elements(By.XPATH, f"//span[contains(text(), '{size_option}')]/ancestor::label")
                    
                    if all_size_options:
                        # 找到了可能的尺寸選項，嘗試點擊第一個
                        size_option_elem = all_size_options[0]
                        driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", size_option_elem)
                        time.sleep(1)
                        driver.execute_script("arguments[0].click();", size_option_elem)
                        print(f"✅ 已通過通用方法選擇杯子尺寸: {size_option}")
                        time.sleep(1)
                    else:
                        # 如果找不到任何尺寸選項，可能此飲料不需要選擇尺寸
                        print(f"⚠️ 未找到任何尺寸選項，此飲料可能不需要選擇杯型，直接跳過")
                except Exception as e2:
                    print(f"⚠️ 通用方法選擇杯子尺寸失敗，可能不需要選擇杯型: {str(e2)}")
                    print("⚠️ 跳過尺寸選擇，繼續處理其他選項")
        else:
            print("⚠️ 跳過杯子尺寸選擇，無尺寸選項")
        ...
        # 3. 選擇食物細項 - 迷客夏特殊處理
        # 處理溫度選項
        if '溫度' in food_options_obj:
            temperature = food_options_obj['溫度']
            try:
        ...
        # 處理甜度選項
        if '甜度' in food_options_obj:
            sweetness = food_options_obj['甜度']
            try:
                # 先找到甜度選項組
        ...
        # 處理加料選項
        for option_key, option_value in food_options_obj.items():
            # 跳過已處理的溫度和甜度選項
    ...
    if scheduled_delivery_clicked:
        ...
        if time_selected and time_selected.get('success', False):
            ...
            confirm_clicked = driver.execute_script(""" 
                // 查找並點擊確認按鈕
                var confirmButton = document.querySelector('button.bds-c-btn.bds-c-btn-primary.bds-c-btn--size-regular');
                if (confirmButton) {
                    console.log('找到確認按鈕');
                    confirmButton.click();
                    return true;
                ...
                // 如果上面的選擇器沒找到,嘗試其他可能的選擇器
                confirmButton = document.querySelector('button[class*="bds-c-btn-primary"]');
                if (confirmButton) {
                    console.log('通過備用選擇器找到確認按鈕');
                    confirmButton.click(); 
                    return true;
                }
                ...
                var buttons = document.querySelectorAll('button');
                for (var i = 0; i < buttons.length; i++) {
                    if (buttons[i].textContent.includes('確認')) {
                        console.log('通過文字內容找到確認按鈕');
                        buttons[i].click();
                        return true;
                    }
                }
