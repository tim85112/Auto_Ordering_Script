...
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome2\UserData_foodpanda_coco')

# ✅ 檢查是否需要首次手動登入
print("\n" + "="*60)
print("Foodpanda 自動訂餐程序 - CoCo")
print("="*60)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    print("   如需強制手動登入，請使用參數執行: python foodpanda_coco.py login")
...
# 讀取訂單資料
order_file_path = "ubereats_coco.json"
...
# 📌 Foodpanda CoCo URL
foodpanda_url = "https://www.foodpanda.com.tw/restaurant/w8b3/cocodu-ke-bei-tou-zhong-he-dian"
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("foodpanda_coco_cookies.pkl"):
        print("⚠️ Cookie文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie文件不存在")
        
    # 讀取Cookie文件
    with open("foodpanda_coco_cookies.pkl", "rb") as file:
    ...
    driver.get(foodpanda_url)  # 重新打開CoCo頁面
    ...
    if login_detected:
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("foodpanda_coco_cookies.pkl", "wb") as file:
...
for order in orders:
    try:
        ...
        # 去掉英文部分（英文部分通常在括號內或空格後）
        menu_item = full_menu_item.split('(')[0].split(' ')[0].strip()
        food_options = {}
        
        # 處理食物細項，將它們轉換為字典格式
        for option in order.get('食物細項', []):
            if '：' in option:  # 使用中文冒號分隔
                key, value = option.split('：', 1)
                # 去掉英文部分
                value = value.split(' ')[0].strip()
                food_options[key] = value
            elif ':' in option:  # 使用英文冒號分隔
                key, value = option.split(':', 1)
                # 去掉英文部分
                value = value.split(' ')[0].strip()
                food_options[key] = value
        ...
        # 應用冰量映射
        if '冰量' in food_options and food_options['冰量'] in ice_mapping:
            food_options['冰量'] = ice_mapping[food_options['冰量']]
            print(f"🔄 已將冰量映射為: {food_options['冰量']}")
            
        # 應用甜度映射
        if '甜度' in food_options and food_options['甜度'] in sugar_mapping:
            food_options['甜度'] = sugar_mapping[food_options['甜度']]
            print(f"🔄 已將甜度映射為: {food_options['甜度']}")
            
        # 應用飲品溫度映射
        if '飲品溫度' in food_options and food_options['飲品溫度'] in temp_mapping:
            food_options['飲品溫度'] = temp_mapping[food_options['飲品溫度']]
            print(f"🔄 已將飲品溫度映射為: {food_options['飲品溫度']}")
        ...
        # 3. 選擇食物細項 - 使用50嵐的方式處理
        # 處理溫度/冰量選項
        if '飲品溫度' in food_options or '冰量' in food_options:
            temperature_key = '飲品溫度' if '飲品溫度' in food_options else '冰量'
            temperature = food_options[temperature_key]
            try:
                # 先找到溫度選項組（可能稱為"冰度選擇"或"冰熱選擇"）
                temp_group = wait.until(EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(@class, 'modifier-group') and (.//div[contains(text(), '溫度') or .//div[contains(text(), '冰度') or .//div[contains(text(), '冰熱') or .//div[contains(text(), '冰量')])]")))
                print(f"✅ 找到溫度/冰度選項組")
                ...
                print(f"✅ 已選擇{temperature_key}: {temperature}")
                time.sleep(1)
            except Exception as e:
                print(f"⚠️ 選擇{temperature_key} {temperature} 時出錯: {str(e)}")
                # 嘗試通用方法
                try:
                    ...
                    time.sleep(1)
                    driver.execute_script("arguments[0].click();", temp_option)
                    print(f"✅ 已通過通用方法選擇{temperature_key}: {temperature}")
                    time.sleep(1)
                except Exception as e2:
                    print(f"❌ 通用方法選擇{temperature_key}失敗: {str(e2)}")
