...
print(f"\n🚀 UberEats 訂單抓取程式 (清心福全 北投光明店) v{VERSION} (更新日期: {LAST_UPDATED})")
print("=" * 50)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    if os.path.exists("ubereats_chinshin_cookies.pkl"):
        try:
            # os.remove("ubereats_chinshin_cookies.pkl")
    ...
    print("   如需強制手動登入，請使用參數執行: python ubereats_chinshin.py login")
...
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome\UserData_ubereats_chinshin')
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("ubereats_chinshin_cookies.pkl"):
        print("⚠️ Cookie 文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie 文件不存在")
    
    try:
        # 載入保存的 Cookie
        with open("ubereats_chinshin_cookies.pkl", "rb") as file:
    ...
    if os.path.exists("ubereats_chinshin_cookies.pkl") and not force_login:
        try:
            # os.remove("ubereats_chinshin_cookies.pkl")
    ...
    if login_detected or (msvcrt.kbhit() and msvcrt.getch().decode('utf-8').lower() == 'y'):
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("ubereats_chinshin_cookies.pkl", "wb") as file:
...
# 🔹 6️⃣ **點擊清心福全 北投光明店**
try:
    # 使用 data-testid 屬性定位清心福全餐廳選項
    milktea_restaurant = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "li[data-testid='menu-item-清心福全 北投光明店']")))
    milktea_restaurant.click()
    print("✅ 成功點擊『清心福全 北投光明店』！")
except Exception as e:
    print(f"❌ 找不到『清心福全 北投光明店』，錯誤：{e}")
    # 嘗試使用 XPath 替代方法
    try:
        milktea_xpath = "//div[contains(text(), '清心福全 北投光明店')]"
        milktea_element = wait.until(EC.element_to_be_clickable((By.XPATH, milktea_xpath)))
        milktea_element.click()
        print("✅ 使用 XPath 成功點擊『清心福全 北投光明店』！")
        ...
        try:
            ...
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].textContent.includes('清心福全 北投光明店')) {
                    elements[i].click();
                    return true;
                }
            }
            ...
            if result:
                print("✅ 使用 JavaScript 成功點擊『清心福全 北投光明店』！")
            else:
                print("❌ 無法找到『清心福全 北投光明店』。")
                
                # 嘗試更廣泛的搜索，只查找包含"清心福全"的元素
                js_script_broader = """
                var elements = document.querySelectorAll('li[role="menuitem"]');
                for (var i = 0; i < elements.length; i++) {
                    if (elements[i].textContent.includes('清心福全')) {
                        console.log('找到清心福全相關餐廳：' + elements[i].textContent);
                        elements[i].click();
                        return true;
                    }
                }
                
                // 如果還找不到，嘗試查找任何可能包含"清心福全"的元素
                var allElements = document.querySelectorAll('div, li, button, span');
                for (var i = 0; i < allElements.length; i++) {
                    if (allElements[i].textContent.includes('清心福全')) {
                        console.log('找到清心福全元素：' + allElements[i].textContent);
                        // 尋找可點擊的父元素
                ...
                if broader_result:
                    print("✅ 使用更廣泛的搜索成功點擊包含『清心福全』的元素！")
                else:
                    print("❌ 即使使用更廣泛的搜索，也無法找到『清心福全』相關元素")
                driver.quit()
                exit()
        except Exception as js_error:
            print(f"❌ JavaScript 點擊失敗，錯誤：{js_error}")
...
try:
    ...
    order_items = driver.execute_script("""
        ...
        detailContainers.forEach((container, index) => {
            try {
                ...
                // 3. 提取食物細項 - 使用改進的方式抓取清心福全的細項
                // 建立食物細項物件
                const foodDetails = {};
                
                // 針對清心福全的訂單項目結構進行處理
                // 查找所有可能包含選項標籤的div元素，通過文本內容辨識
                const allDivs = Array.from(container.querySelectorAll('div'));
                
                // 查找所有標籤文字，如"冰熱選擇"、"糖量選擇"、"茶湯更換選擇"、"加料選擇"等
                for (const div of allDivs) {
                    const divText = div.textContent.trim();
                    // 檢查是否包含關鍵詞
                    if (divText.includes('選擇') && 
                        (divText.includes('冰熱') || 
                         divText.includes('糖量') || 
                         divText.includes('茶湯') || 
                         divText.includes('加料'))) {
                        console.log(`發現選項標籤: ${divText}`);
                        
                        // 提取標籤文字，移除可能的數字
                        let labelText = divText.replace(/\\s*\\d+\\s*($|\\(.+\\))?/g, '').trim();
                        
                        // 查找下一個ul元素及其li
                        const nextUl = div.nextElementSibling;
                        if (nextUl && nextUl.tagName === 'UL') {
                            const liElement = nextUl.querySelector('li');
                            if (liElement) {
                                ...
                                if (valueText !== itemName) {
                                    ...
                                    console.log(`成功提取選項: ${labelText} => ${valueText}`);
                                }
                            }
                        }
                    }
                ...
                // 如果上面的方法未找到食物細項，嘗試備用方法
                if (Object.keys(foodDetails).length === 0) {
                    console.log('基本方法未找到選項，使用備用方法...');
                    ...
                    allUlElements.forEach(ul => {
                        ...
                        if (prevSibling && prevSibling.tagName === 'DIV') {
                            // 獲取ul中的li元素
                    ...
                    // 從配對中提取食物細項
                    divUlPairs.forEach(pair => {
                        // 檢查標籤是否包含關鍵詞
                        const labelKeys = ['冰熱', '糖量', '茶湯', '加料'];
                        const matchedKey = labelKeys.find(key => pair.label.includes(key));
                        
                        if (matchedKey) {
                            // 提取標籤文字，移除可能的數字
                            let labelText = pair.label.replace(/\\s*\\d+\\s*($|\\(.+\\))?/g, '').trim();
                            
                            // 排除餐點名作為選項值
                            if (pair.value !== itemName) {
                                // 存儲到食物細項中
                                foodDetails[labelText] = pair.value;
                                console.log(`備用方法提取選項: ${labelText} => ${pair.value}`);
                            }
                        }
                    });
                }
                ...
                // 最後嘗試：通過直接搜索文本內容查找選項
                if (Object.keys(foodDetails).length === 0) {
                    console.log('嘗試通過文本搜索提取選項...');
                    
                    // 尋找文本中的選項模式：標籤文字+值
                    const optionPatterns = [
                        { key: '冰熱選擇', regex: /冰熱選擇[^]*?[<][^>]*?[>]([^<]+)/i },
                        { key: '糖量選擇', regex: /糖量選擇[^]*?[<][^>]*?[>]([^<]+)/i },
                        { key: '茶湯更換選擇', regex: /茶湯更換選擇[^]*?[<][^>]*?[>]([^<]+)/i },
                        { key: '加料選擇', regex: /加料選擇[^]*?[<][^>]*?[>]([^<]+)/i }
                    ];
                    ...
                    optionPatterns.forEach(pattern => {
                        const match = containerHTML.match(pattern.regex);
                        if (match && match[1]) {
                            const value = match[1].trim();
                            if (value !== itemName) {
                                foodDetails[pattern.key] = value;
                                console.log(`文本搜索提取選項: ${pattern.key} => ${value}`);
                            }
                        }
                    });
                }
            ...
            nameElems.forEach((nameElem, index) => {
                try {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        // 嘗試找到包含選項的容器
                        const hasOptions = container.textContent.includes('選擇') &&
                            (container.textContent.includes('冰熱') || 
                             container.textContent.includes('糖量') || 
                             container.textContent.includes('茶湯') || 
                             container.textContent.includes('加料'));
    ...
    if order_items and 'items' in order_items and len(order_items['items']) > 0:
        ...
        if order_items['總價'] != "總價未知":
            # 將JSON格式化為更容易閱讀的形式
            with open("ubereats_chinshin.json", "w", encoding="utf-8") as f:
                json.dump(output_json, f, ensure_ascii=False, indent=4)
            print("\n✅ 訂單信息已保存到 'ubereats_chinshin.json'")
        else:
            print("\n⚠️ 總價未知，不輸出JSON文件")
    else:
        print("❌ 未能抓取到訂單信息")
