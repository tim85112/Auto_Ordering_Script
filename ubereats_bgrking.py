...
print(f"\n🚀 UberEats 訂單抓取程式 (漢堡王) v{VERSION} (更新日期: {LAST_UPDATED})")
print("=" * 50)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    if os.path.exists("ubereats_bgrking_cookies.pkl"):
        try:
            # os.remove("ubereats_bgrking_cookies.pkl")
    ...
    print("   如需強制手動登入，請使用參數執行: python ubereats_bgrking.py login")
...
# 以下兩行被註釋掉
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome\UserData_ubereats_bgrking')
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("ubereats_bgrking_cookies.pkl"):
        print("⚠️ Cookie 文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie 文件不存在")
    
    try:
        # 載入保存的 Cookie
        with open("ubereats_bgrking_cookies.pkl", "rb") as file:
    ...
    if os.path.exists("ubereats_bgrking_cookies.pkl") and not force_login:
        try:
            # os.remove("ubereats_bgrking_cookies.pkl")
    ...
    if login_detected or (msvcrt.kbhit() and msvcrt.getch().decode('utf-8').lower() == 'y'):
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("ubereats_bgrking_cookies.pkl", "wb") as file:
...
# 🔹 6️⃣ **點擊漢堡王北投光明店**
try:
    # 使用 data-testid 屬性定位漢堡王餐廳選項
    bgrking_restaurant = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "li[data-testid='menu-item-Burger King 漢堡王 北投光明店']")))
    bgrking_restaurant.click()
    print("✅ 成功點擊『Burger King 漢堡王 北投光明店』！")
except Exception as e:
    print(f"❌ 找不到『Burger King 漢堡王 北投光明店』，錯誤：{e}")
    # 嘗試使用 XPath 替代方法
    try:
        bgrking_xpath = "//div[contains(text(), 'Burger King 漢堡王 北投光明店')]"
        bgrking_element = wait.until(EC.element_to_be_clickable((By.XPATH, bgrking_xpath)))
        bgrking_element.click()
        print("✅ 使用 XPath 成功點擊『Burger King 漢堡王 北投光明店』！")
        ...
        try:
            ...
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].textContent.includes('Burger King 漢堡王 北投光明店')) {
                    elements[i].click();
                    return true;
                }
            }
            ...
            if result:
                print("✅ 使用 JavaScript 成功點擊『Burger King 漢堡王 北投光明店』！")
            else:
                print("❌ 無法找到『Burger King 漢堡王 北投光明店』。")
                driver.quit()
                exit()
        except Exception as js_error:
            print(f"❌ JavaScript 點擊失敗，錯誤：{js_error}")
    ...
    patients_data = driver.execute_script("""
        ...
        for (let i = 0; i < patientElements.length; i++) {
            try {
                ...
                // 新增排除條件：系統提示信息、無法供應等文本
                if (patientName.includes('本人') || 
                    patientName.includes('$') || 
                    patientName.length < 2 ||
                    /^\\d+$/.test(patientName) || // 純數字
                    patientName.includes('部分餐點無法供應') ||
                    patientName.includes('無法供應') ||
                    patientName.includes('請移除') ||
                    patientName.includes('無法訂購')) {
...
try:
        ...
        itemNameElements.forEach((nameElem, index) => {
            try {
                ...
                if (priceContainer) {
                    ...
                    // 3. 改進的食物細項解析方法 - 專門針對漢堡王的附餐選擇、飲品選擇、點心加點、加點
                    const foodDetails = {
                        '附餐選擇': [],
                        '飲品選擇': [],
                        '點心加點': [],
                    ...
                    // 直接從訂單區塊中查找所有標題和對應的內容
                    try {
                        // 1. 先尋找所有可能包含食物選項的區域 (整個訂單項目容器)
                        const orderItemContainer = priceContainer;
                        
                        // 2. 查找所有可能的選項標題元素
                        // 使用多種選擇器來增加匹配率
                        const optionTitleSelectors = [
                            'div.b1.ob', // 標準類名選擇器
                            'div.bo.ei.bq.ds.b1.ob', // 更具體的類名選擇器
                            'div[class*="ob"]', // 包含特定類的元素
                            'div:not([data-baseweb="typo-labelmedium"]):not([data-testid])' // 排除已知的其他元素
                        ];
                        
                        // 合併所有選擇器結果
                        let allOptionTitles = [];
                        for (const selector of optionTitleSelectors) {
                            const elements = orderItemContainer.querySelectorAll(selector);
                            elements.forEach(el => {
                                // 只選擇包含關鍵詞的元素
                                const text = el.textContent.trim();
                                if (text.includes('選擇') || text.includes('加點')) {
                                    allOptionTitles.push(el);
                                }
                            });
                        }
                        console.log(`找到 ${allOptionTitles.length} 個可能的選項標題元素`);
                        
                        // 對每個標題元素處理
                        allOptionTitles.forEach(titleElem => {
                            const titleText = titleElem.textContent.trim();
                            console.log(`處理標題: ${titleText}`);
                            
                            // 根據標題文本確定類型
                            let optionType = '';
                            if (titleText.includes('附餐選擇')) {
                                optionType = '附餐選擇';
                            } else if (titleText.includes('飲品選擇') || titleText.includes('飲料選擇') || titleText.includes('Choose Beverage')) {
                                optionType = '飲品選擇';
                            } else if (titleText.includes('點心加點')) {
                                optionType = '點心加點';
                            } else if (titleText.includes('加點')) {
                                optionType = '加點';
                            }
                            
                            // 僅處理識別出類型的標題
                            if (optionType) {
                                console.log(`找到選項類型: ${optionType}`);
                                
                                // 方法1: 查找緊跟在標題後的 UL 列表
                                let nextElement = titleElem.nextElementSibling;
                                if (nextElement && nextElement.tagName === 'UL') {
                                    const listItems = nextElement.querySelectorAll('li');
                                    if (listItems.length > 0) {
                                        listItems.forEach(item => {
                                            const itemText = item.textContent.trim();
                                            if (itemText && !foodDetails[optionType].includes(itemText)) {
                                                foodDetails[optionType].push(itemText);
                                            }
                                        });
                                        console.log(`- 從UL列表找到項目: ${foodDetails[optionType]}`);
                                    }
                                }
                                // 方法2: 從標題元素的父元素中查找所有相關的項目
                                else {
                                    // 查找標題後續的所有元素，直到下一個標題或結束
                                    let currentElem = titleElem.nextElementSibling;
                                    while (currentElem) {
                                        // 如果遇到另一個標題，停止尋找
                                        if (currentElem.textContent.includes('選擇') || 
                                            currentElem.textContent.includes('加點')) {
                                            // 檢查是否是標題元素
                                            const isTitle = currentElem.className.includes('ob') || 
                                                            (currentElem.className.includes('b1') && 
                                                             !currentElem.className.includes('eq'));
                                            if (isTitle) break;
                                        }
                                        
                                        const elemText = currentElem.textContent.trim();
                                        // 確認是項目文本，不是標題或其他元素
                                        if (elemText && 
                                            !elemText.includes('選擇') && 
                                            !elemText.includes('加點') &&
                                            !elemText.includes('$') &&
                                            elemText.length > 0) {
                                            
                                            // 使用類名來識別具體的項目元素
                                            const isItemElement = currentElem.className.includes('eq') || 
                                                                   (currentElem.tagName === 'LI') ||
                                                                   (!currentElem.className.includes('ob'));
                                            
                                            if (isItemElement && !foodDetails[optionType].includes(elemText)) {
                                                foodDetails[optionType].push(elemText);
                                                console.log(`- 找到項目: ${elemText}`);
                                            }
                                        }
                                ...
                                const listItemSelectors = [
                                    'li.bo.ei.bq.ds.eq', // 典型的列表項選擇器
                                    'li[class*="eq"]',   // 包含特定類的列表項
                                    'div[class*="eq"]'   // 某些情況下可能使用div代替li
                                ];
                                
                                // 在標題的父元素內查找所有可能的列表項
                                const parentContainer = titleElem.parentElement;
                                if (parentContainer) {
                                    for (const selector of listItemSelectors) {
                                        const listItems = parentContainer.querySelectorAll(selector);
                        ...
                        // 備用方法: 在整個容器內查找所有可能的食物選項列表項
                        const allListItems = orderItemContainer.querySelectorAll('li.bo.ei.bq.ds.eq, li[class*="eq"]');
                        if (allListItems.length > 0) {
                            console.log(`找到 ${allListItems.length} 個可能的列表項元素`);
                            ...
                            allListItems.forEach(item => {
                                ...
                                while (prev) {
                                    ...
                                    if (text.includes('附餐選擇')) { currentType = '附餐選擇'; break; }
                                    else if (text.includes('飲品選擇') || text.includes('飲料選擇') || text.includes('Choose Beverage')) { 
                                        currentType = '飲品選擇'; 
                                        break; 
                                    }
                                    else if (text.includes('點心加點')) { currentType = '點心加點'; break; }
                                    else if (text.includes('加點') && !text.includes('點心')) { currentType = '加點'; break; }
                                    
                                    prev = prev.previousElementSibling;
                                }
                                
                                // 如果找到類型，添加項目
                                if (currentType && foodDetails[currentType]) {
                                    const itemText = item.textContent.trim();
                                    if (itemText && !foodDetails[currentType].includes(itemText)) {
                                        foodDetails[currentType].push(itemText);
                                        console.log(`- 備用方法找到 ${currentType} 項目: ${itemText}`);
                                    }
                                }
                            });
                        }
                        
                        // 特殊處理：尋找所有可能包含 "華年達橘子味汽水" 或其他特殊飲料的元素
                        // 這是為了補充抓取即使在其他區塊中提及的飲料選擇
                        const allElements = orderItemContainer.querySelectorAll('*');
                        allElements.forEach(elem => {
                            const text = elem.textContent.trim();
                            // 檢查是否包含常見的飲料關鍵詞
                            if ((text.includes('Mirinda') || 
                                 text.includes('華年達') || 
                                 text.includes('百事') || 
                                 text.includes('可樂') || 
                                 text.includes('雪碧') || 
                                ...
                                // 確認這是一個飲料選項
                                if (!foodDetails['飲品選擇'].includes(text)) {
                                    foodDetails['飲品選擇'].push(text);
                                    console.log(`- 找到額外飲料選項: ${text}`);
                                }
                            }
                        });
                        
                    } catch (error) {
                        console.error('處理食物細項時出錯:', error);
                    }
                    
                    // 確保飲料選擇不為空 - 如果為空但在附餐選擇中已提及飲料，則進行特殊處理
                    if (foodDetails['飲品選擇'].length === 0) {
                        // 檢查附餐選擇中是否已經包含飲料信息
                        const comboText = foodDetails['附餐選擇'].join(' ');
                        if (comboText.includes('百事可樂') || comboText.includes('Pepsi')) {
                            // 在整個容器中搜索可能的飲料選擇
                            const allTexts = [];
                            ...
                            for (const text of allTexts) {
                                if ((text.includes('華年達') || 
                                     text.includes('Mirinda') || 
                                     text.includes('雪碧') || 
                                     text.includes('Sprite') || 
                                     text.includes('汽水') ||
                                     (text.includes('百事') && !comboText.includes(text)) ||
                                     (text.includes('Pepsi') && !comboText.includes(text))) && 
                                    !text.includes('附餐') && 
                                    !text.includes('選擇') && 
                                    !text.includes('加點')) {
                                    
                                    foodDetails['飲品選擇'].push(text);
                                    console.log(`- 從文本節點找到飲料選項: ${text}`);
                                    break;
                                }
                            }
                        }
                    ...
                    // 合併所有食物細項為一個數組，但改為對象格式來保存
                    const foodDetailsObj = {};
                    for (const [type, items] of Object.entries(foodDetails)) {
                        if (items.length > 0) {
                            foodDetailsObj[type] = items.join(', ');
                        }
                    }
                    
                    // 將對象轉換為鍵值對列表格式，與原來保持一致
                    const foodDetailsList = [];
                    for (const [type, itemsText] of Object.entries(foodDetailsObj)) {
                        foodDetailsList.push(`${type}：${itemsText}`);
                    }
                    
                    console.log(`最終解析的食物細項:`, foodDetailsObj);
    ...
    if order_items and 'items' in order_items and len(order_items['items']) > 0:
        ...
        with open("ubereats_bgrking.json", "w", encoding="utf-8") as f:
            json.dump(output_json, f, ensure_ascii=False, indent=4)
        print("\n✅ 訂單信息已保存到 'ubereats_bgrking.json'")
    else:
        print("❌ 未能抓取到訂單信息")
