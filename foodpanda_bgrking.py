...
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome2\UserData_foodpanda_bgrking')
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    print("   如需強制手動登入，請使用參數執行: python foodpanda_bgrking.py login")
...
# 讀取訂單資料
order_file_path = "ubereats_bgrking.json"
...
# 📌 Foodpanda 漢堡王 URL
foodpanda_url = "https://www.foodpanda.com.tw/restaurant/qhsy/burger-king-han-bao-wang-tai-bei-shi-lin-dian"
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("foodpanda_bgrking_cookies.pkl"):
        print("⚠️ Cookie文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie文件不存在")
        
    # 讀取Cookie文件
    with open("foodpanda_bgrking_cookies.pkl", "rb") as file:
    ...
    try:
        # 等待 10 秒檢查是否有登入狀態的元素 (增加檢查時間)
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, "button.cart-button, svg[data-testid='personal-icon']"))
        )
        print("✅ 成功使用 Cookie 登入")
    except:
        # 如果沒有找到登入狀態元素，則視為登入失敗，需要重新登入
    ...
    while time.time() - start_time < login_timeout:
        ...
        try:
            ...
            # 方法2: 檢查購物車按鈕
            cart_button = driver.find_elements(By.CSS_SELECTOR, 'button.cart-button')
            
            # 方法3: 檢查是否不再顯示登入按鈕
            login_button = driver.find_elements(By.XPATH, "//button[contains(text(), '登入') or contains(text(), 'Login') or contains(text(), 'Sign in')]")
            login_button_not_visible = len(login_button) == 0
            
            # 更嚴格的登入檢測: 確保至少兩個條件，避免誤判
            ...
            if login_button_not_visible:
                login_conditions_met += 1
                print("✓ 登入按鈕已消失")
            
            # 如果滿足至少兩個條件，才認為已登入
            if login_conditions_met >= 2:
    ...
    if login_detected:
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("foodpanda_bgrking_cookies.pkl", "wb") as file:
...
def expand_food_options():
    """展開食物選項頁面中的所有"View more"按鈕"""
    try:
        print("⏳ 嘗試展開所有食物選項...")
        # 使用JavaScript查找並點擊所有"View more"按鈕
        view_buttons_clicked = driver.execute_script("""
            ...
            // 找出所有包含"View"和"more"的按鈕
            for (var i = 0; i < allButtons.length; i++) {
                var buttonText = allButtons[i].textContent.trim();
                if (buttonText.includes('View') && buttonText.includes('more')) {
                    console.log('找到View more按鈕: ' + buttonText);
                    viewButtons.push(allButtons[i]);
                }
            }
            
            console.log('找到 ' + viewButtons.length + ' 個View more按鈕');
            ...
            for (var j = 0; j < viewButtons.length; j++) {
                try {
                    ...
                    // 直接點擊按鈕（不使用setTimeout）
                    try {
                        // 確認按鈕可見且可點擊
                        if (viewButtons[j].offsetParent !== null && 
                            !viewButtons[j].disabled && 
                            viewButtons[j].style.display !== 'none' && 
                            viewButtons[j].style.visibility !== 'hidden') {
                            
                            console.log('點擊View more按鈕: ' + viewButtons[j].textContent);
                            viewButtons[j].click();
                            clickCount++;
                        }
                    } catch (clickErr) {
        ...
        if view_buttons_clicked and view_buttons_clicked.get('found', 0) > 0:
            print(f"✅ 找到 {view_buttons_clicked.get('found')} 個View more按鈕，成功點擊了 {view_buttons_clicked.get('clicked')} 個")
            # 給按鈕點擊和展開一些時間
            time.sleep(3)
            
            # 檢查是否所有按鈕都成功點擊
            if view_buttons_clicked.get('clicked', 0) < view_buttons_clicked.get('found', 0):
                print("⚠️ 部分按鈕未成功點擊，嘗試再次點擊...")
                # 再次嘗試展開未被點擊的按鈕
                driver.execute_script("""
                    var remainingButtons = document.querySelectorAll('button');
                    for (var i = 0; i < remainingButtons.length; i++) {
                        var buttonText = remainingButtons[i].textContent.trim();
                        if (buttonText.includes('View') && buttonText.includes('more')) {
                            console.log('再次嘗試點擊: ' + buttonText);
                            remainingButtons[i].scrollIntoView({block: 'center'});
                            try {
                                remainingButtons[i].click();
                            } catch (e) {
                                console.log('點擊失敗: ' + e);
                            }
                        }
            ...
            # 檢查飲品選項是否已展開
            drink_options = driver.find_elements(By.XPATH, "//label[contains(., '紅茶') or contains(., '綠茶') or contains(., '可樂')]")
            if drink_options:
                print(f"✅ 已成功展開飲品選項，找到 {len(drink_options)} 個飲品選擇")
            else:
                print("⚠️ 可能未成功展開所有選項，嘗試點擊任何剩餘的展開按鈕...")
                # 使用單獨的方法再嘗試一次
                try:
                    remaining_buttons = driver.find_elements(By.XPATH, "//button[contains(text(), 'View') and contains(text(), 'more')]")
                    for button in remaining_buttons:
                        driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", button)
                        time.sleep(0.5)
                        driver.execute_script("arguments[0].click();", button)
                        print(f"已點擊剩餘按鈕: '{button.text}'")
                        time.sleep(1)
                except Exception as e:
                    print(f"點擊剩餘按鈕時出錯: {str(e)}")
                    
            return True
        else:
            print("⚠️ 未找到任何View more按鈕，可能已全部展開或頁面結構不同")
            return False
...
for order in orders:
    try:
        # 獲取當前訂單的餐點名稱 (去除英文部分和括號内的中文)
        full_menu_item = order.get('餐點', '')
        # 去除英文部分，只保留中文部分 (假設中文在前，英文在后)
        menu_item = full_menu_item.split(' ')[0] if ' ' in full_menu_item else full_menu_item
        # 去除括号内容
        if '(' in menu_item:
        ...
        # 6. 先检查是否有套餐选项需要选择
        has_set_meal = False
        set_meal_option = None
        
        # 查找是否存在套餐選項
        for option in food_options:
            if "附餐選擇：" in option:
                has_set_meal = True
                set_meal_option = option
                print(f"✅ 找到套餐選項: {set_meal_option}")
                break
        
        # 如果有套餐選項，先處理它
        if has_set_meal:
            try:
                # 從選項中獲取套餐字母 (例如從"附餐選擇：B 套餐"獲取"B")
                set_meal_letter = set_meal_option.split("附餐選擇：")[1].split(" ")[0]
                print(f"✅ 需要選擇的套餐: {set_meal_letter} 套餐")
                
                # 使用多種模式匹配查找套餐选项
                set_meal_element = None
                
                # 嘗試多種可能的套餐格式
                set_meal_patterns = [
                    f"{set_meal_letter} 套餐",  # "A 套餐"
                    f"{set_meal_letter}套餐",   # "A套餐"
                    f"{set_meal_letter}餐",     # "A餐"
                    f"{set_meal_letter}-餐",    # "A-餐"
                    f"{set_meal_letter} 餐",    # "A 餐"
                    f"套餐{set_meal_letter}",   # "套餐A"
                    f"餐{set_meal_letter}"      # "餐A"
                ]
                
                print(f"✅ 嘗試匹配以下套餐模式: {set_meal_patterns}")
                
                # 第一輪：嘗試精確匹配
                for pattern in set_meal_patterns:
                    set_meal_elements = driver.find_elements(By.XPATH, 
                        f"//label[contains(text(), '{pattern}') or .//span[contains(text(), '{pattern}')]]")
                    if set_meal_elements:
                        set_meal_element = set_meal_elements[0]
                        print(f"✅ 使用精確模式 '{pattern}' 找到套餐選項")
                        break
                
                # 第二輪：如果精確匹配失敗，則嘗試更寬松的匹配
                if not set_meal_element:
                    xpath_query = "//label["
                    for i, pattern in enumerate(set_meal_patterns):
                        if i > 0:
                            xpath_query += " or "
                        xpath_query += f"contains(., '{pattern}')"
                    xpath_query += "]"
                    
                    set_meal_elements = driver.find_elements(By.XPATH, xpath_query)
                    if set_meal_elements:
                        set_meal_element = set_meal_elements[0]
                        print(f"✅ 使用複合模式找到套餐選項")
                
                # 第三輪：如果仍然失敗，則嘗試僅匹配字母
                if not set_meal_element:
                    print(f"⚠️ 未通過套餐模式找到匹配，嘗試直接查找包含 '{set_meal_letter}' 的選項")
                    # 查找所有包含該字母的label元素
                    letter_elements = driver.find_elements(By.XPATH, 
                        f"//label[contains(., '{set_meal_letter}')]")
                    
                    # 過濾可能的套餐選項
                    filtered_elements = []
                    for elem in letter_elements:
                        elem_text = elem.text.lower()
                        # 檢查元素文本是否包含"套餐"、"餐"或套餐字母在文本開頭
                        if "套餐" in elem_text or "餐" in elem_text or elem_text.startswith(set_meal_letter.lower()):
                            filtered_elements.append(elem)
                    
                    if filtered_elements:
                        set_meal_element = filtered_elements[0]
                        print(f"✅ 通過字母匹配找到可能的套餐選項: '{set_meal_element.text}'")
                
                # 點擊找到的套餐選項
                if set_meal_element:
                    driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", set_meal_element)
                    time.sleep(1)
                    driver.execute_script("arguments[0].click();", set_meal_element)
                    print(f"✅ 已選擇套餐: {set_meal_element.text}")
                else:
                    print(f"⚠️ 未找到 {set_meal_letter} 相關套餐選項，嘗試最後一種方法")
                    
                    # 最後嘗試：查找任何包含套餐字樣的選項並點擊第一個
                    try:
                        # 使用JavaScript來尋找並點擊可能的套餐選項
                        meal_clicked = driver.execute_script("""
                            // 查找所有可能的套餐標籤
                            var allLabels = document.querySelectorAll('label');
                            var mealLabels = [];
                            
                            // 遍歷所有標籤，找出包含"套餐"或"餐"的選項
                            for (var i = 0; i < allLabels.length; i++) {
                                var labelText = allLabels[i].textContent.toLowerCase();
                                if (labelText.includes('套餐') || labelText.includes('餐')) {
                                    mealLabels.push(allLabels[i]);
                                }
                            }
                            
                            console.log('找到 ' + mealLabels.length + ' 個可能的套餐選項');
                            
                            // 如果找到了套餐選項，點擊第一個
                            if (mealLabels.length > 0) {
                                var firstLabel = mealLabels[0];
                                ...
                                setTimeout(function() {
                                    try {
                                        firstLabel.click();
                                        return {success: true, text: firstLabel.textContent};
                                    } catch (err) {
                            ...
                            return {success: false, message: '未找到任何套餐選項'};
                        """)
                        
                        if meal_clicked and meal_clicked.get('success', False):
                            print(f"✅ 已通過JavaScript選擇套餐: {meal_clicked.get('text', '未知套餐')}")
                        else:
                            error_msg = meal_clicked.get('error', meal_clicked.get('message', '未知原因'))
                            print(f"❌ JavaScript選擇套餐失敗: {error_msg}")
                    except Exception as js_err:
                        print(f"❌ 執行JavaScript尋找套餐選項時出錯: {str(js_err)}")
                        
                        # 如果JavaScript方法也失敗，嘗試原始方法最後一次
                        broader_elements = driver.find_elements(By.XPATH, 
                            "//label[contains(., '套餐') or contains(., '餐')]")
                        if broader_elements:
                            driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", broader_elements[0])
                            time.sleep(1)
                            driver.execute_script("arguments[0].click();", broader_elements[0])
                            print(f"✅ 已選擇第一個可用的套餐選項: {broader_elements[0].text}")
                        else:
                            print(f"❌ 無法找到任何套餐相關選項")
                
                time.sleep(2)  # 等待套餐選擇生效
            except Exception as e:
                print(f"❌ 選擇套餐選項時出錯: {str(e)}")
        ...
        for option in food_options:
            if "飲品選擇：" in option:
                drink_option = option
                drink_name = drink_option.split("飲品選擇：")[1].split("，")[0]
                print(f"✅ 找到飲品選項: {drink_name}")
                
                # 查找並選擇飲品
                try:
                    # 先查找相應的飲品類別
                    drink_headers = driver.find_elements(By.XPATH, "//span[contains(text(), '飲品') or contains(text(), '套餐') and contains(text(), '飲料')]")
                    
                    if drink_headers:
                        # 展開飲品區域
                        for header in drink_headers:
                            try:
                                parent_element = header.find_element(By.XPATH, "./..")
                                driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", parent_element)
                                time.sleep(0.5)
                            except Exception as e:
                                print(f"滾動到飲品標題時出錯: {str(e)}")
                    
                    # 嘗試找到飲品選項
                    drink_elements = driver.find_elements(By.XPATH, f"//label[contains(., '{drink_name}')]")
                    if drink_elements:
                        # 滾動到飲品選項
                        driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", drink_elements[0])
                        time.sleep(1)
                        # 點擊飲品選項
                        driver.execute_script("arguments[0].click();", drink_elements[0])
                        print(f"✅ 已選擇飲品: {drink_name}")
                    else:
                        print(f"⚠️ 未找到 {drink_name} 飲品選項，嘗試更模糊的匹配")
                        
                        # 嘗試更模糊的匹配
                        broader_elements = None
                        if "綠茶" in drink_name:
                            broader_elements = driver.find_elements(By.XPATH, "//label[contains(., '綠茶')]")
                        elif "紅茶" in drink_name:
                            broader_elements = driver.find_elements(By.XPATH, "//label[contains(., '紅茶')]")
                        elif "可樂" in drink_name:
                            broader_elements = driver.find_elements(By.XPATH, "//label[contains(., '可樂')]")
                        else:
                            # 分割飲品名稱
                            drink_keyword = drink_name.split(" ")[0]
                            broader_elements = driver.find_elements(By.XPATH, f"//label[contains(., '{drink_keyword}')]")
                        ...
                        if broader_elements:
                            ...
                            # 點擊飲品選項
                            driver.execute_script("arguments[0].click();", broader_elements[0])
                            print(f"✅ 通過模糊匹配選擇了飲品: {drink_name}")
                        else:
                            print(f"❌ 無法找到 {drink_name} 相關飲品選項")
                except Exception as e:
                    print(f"❌ 選擇飲品選項時出錯: {str(e)}")
                break
        ...
        for option in food_options:
            if "加點：" in option:
                addon_option = option
                addon_name = addon_option.split("加點：")[1].split("，")[0]
                print(f"✅ 找到加點選項: {addon_name}")
                
                # 查找並選擇加點項目
                try:
                    # 嘗試找到加點選項
                    addon_elements = driver.find_elements(By.XPATH, f"//label[contains(., '{addon_name}')]")
                    if addon_elements:
                        # 滾動到加點選項
                        driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", addon_elements[0])
                        time.sleep(1)
                        # 點擊加點選項
                        driver.execute_script("arguments[0].click();", addon_elements[0])
                        print(f"✅ 已選擇加點項目: {addon_name}")
                    else:
                        print(f"⚠️ 未找到 {addon_name} 加點選項，嘗試更模糊的匹配")
                        
                        # 嘗試更模糊的匹配
                        found_addon = False
                        addon_keywords = addon_name.split(" ")
                        for keyword in addon_keywords:
                            if len(keyword) > 1:  # 避免太短的關鍵詞
                                broader_elements = driver.find_elements(By.XPATH, f"//label[contains(., '{keyword}')]")
                                if broader_elements:
                                    # 滾動到加點選項
                                    driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", broader_elements[0])
                                    time.sleep(1)
                                    # 點擊加點選項
                                    driver.execute_script("arguments[0].click();", broader_elements[0])
                                    print(f"✅ 通過關鍵詞 '{keyword}' 模糊匹配選擇了加點項目")
                                    found_addon = True
                                    break
                        
                        if not found_addon:
                            print(f"❌ 無法找到 {addon_name} 相關加點選項")
                except Exception as e:
                    print(f"❌ 選擇加點項目時出錯: {str(e)}")
                break
        
        # 10. 處理其他食物細項 (除了已處理的套餐、飲品和加點選項)
        for option in food_options:
            # 跳過已處理的選項
            if "附餐選擇：" in option or "飲品選擇：" in option or "加點：" in option:
                continue
            ...
            try:
                ...
                # 去除"選項："前缀
                if "選項：" in option:
                    option_text = option.split("選項：")[1]
                
                # 分离中英文，只保留中文部分
                chinese_part = ""
                # 如果包含括号，优先提取括号内的中文
                if "(" in option_text and ")" in option_text:
                    paren_content = option_text.split("(")[1].split(")")[0]
                    # 检查括号内容是否包含中文（例如"辣"）
                    if any('\u4e00' <= char <= '\u9fff' for char in paren_content):
                        chinese_part = paren_content
                
                # 如果括号中没有有效的中文，则尝试分离整个字符串中的中文部分
                if not chinese_part:
                    # 分离空格分隔的部分
                ...
                if not chinese_part:
                    # 检查是否有空格，如果有则取第一部分
                    if " " in option_text:
                        chinese_part = option_text.split(" ")[0]
                    else:
                        chinese_part = option_text
                
                # 提取关键词，例如从"辣椒醬 (辣)"提取"辣椒醬"或"辣"
                search_keywords = []
                
                # 添加原始文本的关键部分
                if chinese_part:
                    search_keywords.append(chinese_part)
                
                # 如果是辣椒醬之类的选项，添加更多关键词
                if "辣椒" in option_text or "辣" in option_text:
                    search_keywords.extend(["辣椒", "辣"])
                elif "不辣" in option_text:
                    search_keywords.append("不辣")
                elif "酸菜" in option_text:
                    search_keywords.append("酸菜")
                elif "青蔥" in option_text or "蔥" in option_text:
                    search_keywords.extend(["青蔥", "蔥"])
                
                # 移除空字符串和太短的关键词
                search_keywords = [kw for kw in search_keywords if len(kw) > 1]
                
                print(f"處理選項: {option}，搜索關鍵詞: {search_keywords}")
                
                # 尝试匹配每个关键词
                found_option = False
                for keyword in search_keywords:
                    # 使用模糊匹配找到选项
                    option_elements = driver.find_elements(By.XPATH, f"//*[contains(text(), '{keyword}')]/ancestor::label")
                    
                    if option_elements:
                        driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", option_elements[0])
                        time.sleep(1)
                        driver.execute_script("arguments[0].click();", option_elements[0])
                        print(f"✅ 已選擇選項: '{option_elements[0].text}' (通過關鍵詞: '{keyword}')")
                        found_option = True
                        break
                
                # 如果没有找到选项，尝试更通用的方法
                if not found_option:
                    print(f"⚠️ 未通過關鍵詞找到匹配，嘗試其他方法")
                    
                    # 尝试使用JavaScript查找并点击选项
                    option_clicked = driver.execute_script("""
                        // 提取关键词
                        var keyword = arguments[0];
                        var option = arguments[1];
                        console.log('尝试查找选项: ' + option + '，关键词: ' + keyword);
                        ...
                        for (var i = 0; i < allLabels.length; i++) {
                            ...
                            if (labelText.includes(keyword.toLowerCase())) {
                                matchedLabels.push(allLabels[i]);
                            }
                        }
                        
                        console.log('找到 ' + matchedLabels.length + ' 个可能的匹配项');
                        
                        // 如果找到了匹配项，点击第一个
                        if (matchedLabels.length > 0) {
                            var label = matchedLabels[0];
                    ...
                    """, search_keywords[0] if search_keywords else option_text, option)
                    
                    if option_clicked and option_clicked.get('success', False):
                        print(f"✅ 已通過JavaScript選擇選項: {option_clicked.get('text', '未知選項')}")
                    else:
                        # 最後嘗試：查找任何可能相關的標籤
                        # 尝试查找原始选项文本中的每个字符（适用于中文）
                        for char in option_text:
                            if '\u4e00' <= char <= '\u9fff':  # 是否是中文字符
                                char_elements = driver.find_elements(By.XPATH, f"//label[contains(., '{char}')]")
                                if char_elements:
                                    driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", char_elements[0])
                                    time.sleep(1)
                                    driver.execute_script("arguments[0].click();", char_elements[0])
                                    print(f"✅ 已通過單個字符 '{char}' 選擇選項: {char_elements[0].text}")
                                    found_option = True
                                    break
                        
                        if not found_option:
                            print(f"❌ 無法找到與 '{option}' 相匹配的選項")
                
                time.sleep(1)  # 給選項選擇一些間隔時間
            except Exception as e:
                print(f"⚠️ 處理選項 {option} 時出錯: {str(e)}")
