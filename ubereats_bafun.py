...
print("\n" + "=" * 50)
print(f"UberEats 訂單抓取程式 (八方雲集) v{VERSION} (更新日期: {LAST_UPDATED})")
print("=" * 50)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    if os.path.exists("ubereats_bafun_cookies.pkl"):
        try:
            # os.remove("ubereats_bafun_cookies.pkl")
    ...
    print("   如需強制手動登入，請使用參數執行: python ubereats_bafun.py login")
...
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome\UserData_ubereats_bafun')
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("ubereats_bafun_cookies.pkl"):
        print("⚠️ Cookie 文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie 文件不存在")
    
    try:
        # 載入保存的 Cookie
        with open("ubereats_bafun_cookies.pkl", "rb") as file:
    ...
    if os.path.exists("ubereats_bafun_cookies.pkl") and not force_login:
        try:
            # os.remove("ubereats_bafun_cookies.pkl")
    ...
    if login_detected or (msvcrt.kbhit() and msvcrt.getch().decode('utf-8').lower() == 'y'):
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("ubereats_bafun_cookies.pkl", "wb") as file:
...
# 🔹 6️⃣ **點擊八方雲集 北投中和店**
try:
    # 使用 data-testid 屬性定位八方雲集餐廳選項
    bafun_restaurant = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "li[data-testid='menu-item-八方雲集 北投中和店']")))
    bafun_restaurant.click()
    print("✅ 成功點擊『八方雲集 北投中和店』！")
except Exception as e:
    print(f"❌ 找不到『八方雲集 北投中和店』，錯誤：{e}")
    # 嘗試使用 XPath 替代方法
    try:
        bafun_xpath = "//div[contains(text(), '八方雲集 北投中和店')]"
        bafun_element = wait.until(EC.element_to_be_clickable((By.XPATH, bafun_xpath)))
        bafun_element.click()
        print("✅ 使用 XPath 成功點擊『八方雲集 北投中和店』！")
        ...
        try:
            ...
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].textContent.includes('八方雲集 北投中和店')) {
                    elements[i].click();
                    return true;
                }
            }
            ...
            if result:
                print("✅ 使用 JavaScript 成功點擊『八方雲集 北投中和店』！")
            else:
                print("❌ 無法找到『八方雲集 北投中和店』。")
                driver.quit()
                exit()
        except Exception as js_error:
            print(f"❌ JavaScript 點擊失敗，錯誤：{js_error}")
...
try:
        ...
        itemNameElements.forEach((nameElem, index) => {
            try {
                ...
                if (priceContainer) {
                    ...
                    // 3. 解析餐點選項 - 改進後的方法，更精準地捕捉食物細項
                    const options = [];
                    
                    // 查找所有可能的選項容器 - 首先尋找固定類別名稱的元素
                    const optionTitles = priceContainer.querySelectorAll('div.bo.ei.bq.ds.b1.y0');
                    const optionLists = priceContainer.querySelectorAll('ul');
                    
                    // 處理選項標題和對應的列表
                    for(let i = 0; i < optionTitles.length; i++) {
                        const titleElem = optionTitles[i];
                        const title = titleElem.textContent.trim();
                        
                        // 查找對應的選項列表
                        let optionList = null;
                        // 檢查緊接著的下一個元素是否為ul
                        if(titleElem.nextElementSibling && titleElem.nextElementSibling.tagName === 'UL') {
                            optionList = titleElem.nextElementSibling;
                        } 
                        // 如果標題後沒有直接跟著列表，則嘗試查找附近的列表
                        else if(i < optionLists.length) {
                            optionList = optionLists[i];
                        }
                        
                        if(optionList) {
                            // 從列表中獲取所有選項值
                            const optionItems = Array.from(optionList.querySelectorAll('li'))
                    ...
                    // 備用方案: 如果上述方法未找到選項，尋找可能的選項值
                    if(options.length === 0) {
                        // 尋找所有可能的選項值元素
                        const optionElements = priceContainer.querySelectorAll('li.bo.ei.bq.ds.eq');
                        if(optionElements.length > 0) {
                            // 查找每個選項值的標題
                            optionElements.forEach(optElem => {
                                // 嘗試向上查找相對應的標題
                                let currentElement = optElem.parentElement;
                                ...
                                for(let i = 0; i < 3; i++) {
                                    ...
                                    // 查找是否有標題元素
                                    const titleElements = currentElement.querySelectorAll('div.bo.ei.bq.ds.b1.y0');
                                    if(titleElements.length > 0) {
                                        optionTitle = titleElements[0].textContent.trim();
                                        foundTitle = true;
                                        break;
                                    }
                                ...
                                options.push({
                                    title: optionTitle,
                                    value: optElem.textContent.trim()
                                });
                            });
                        }
                    }
                    ...
                    for(const opt of options) {
                        if(opt.title && opt.value) {
                            // 對於八方雲集特有的格式，採用 "標題：值" 的形式
                            optionValues.push(`${opt.title}：${opt.value}`);
                        } else if(opt.value) {
    ...
    if order_items and 'items' in order_items and len(order_items['items']) > 0:
        ...
        with open("ubereats_bafun.json", "w", encoding="utf-8") as f:
            json.dump(output_json, f, ensure_ascii=False, indent=4)
        print("\n✅ 訂單信息已保存到 'ubereats_bafun.json'")
    else:
        print("❌ 未能抓取到訂單信息")
