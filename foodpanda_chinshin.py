...
options.add_argument(r'--user-data-dir=C:\Users\ivan\AppData\Local\Google\Chrome2\UserData_foodpanda_chinshin')

# ✅ 檢查是否需要首次手動登入
print("\n" + "="*60)
print("Foodpanda 清心福全自動訂餐程序")
print("="*60)
...
if len(sys.argv) > 1 and sys.argv[1].lower() == 'login':
    ...
    print("   如需強制手動登入，請使用參數執行: python foodpanda_chinshin.py login")
...
# 讀取訂單資料
order_file_path = "ubereats_chinshin.json"
...
# 📌 Foodpanda 清心福全 URL
foodpanda_url = "https://www.foodpanda.com.tw/restaurant/im3j/qing-xin-fu-quan-bei-tou-guang-ming-dian"
...
try:
    ...
    # 檢查 Cookie 文件是否存在
    if not os.path.exists("foodpanda_chinshin_cookies.pkl"):
        print("⚠️ Cookie文件不存在，需要手動登入")
        raise FileNotFoundError("Cookie文件不存在")
        
    # 讀取Cookie文件
    with open("foodpanda_chinshin_cookies.pkl", "rb") as file:
    ...
    driver.get(foodpanda_url)  # 重新打開清心福全頁面
    ...
    if login_detected:
        ...
        if cookies:
            ...
            try:
                ...
                # 保存有效的 cookies
                with open("foodpanda_chinshin_cookies.pkl", "wb") as file:
...
for order in orders:
    try:
        # 獲取當前訂單的餐點名稱
        menu_item_full = order.get('餐點', '')
        
        # 使用正則表達式找到英文部分
        # 將以空格開始，跟著英文字母、數字、標點符號的部分替換為空
        menu_item = re.sub(r'\s+[A-Za-z0-9\s().,]+$', '', menu_item_full)
        ...
        if '溫度' in food_options:
            ...
            try:
                print(f"🔍 處理冰熱選擇: {temperature}")
                
                # 簡化的模糊匹配邏輯 - 提取關鍵詞
                temperature_keywords = re.sub(r'\s*\([^)]*\)', '', temperature).strip()  # 去除括號內容
                temperature_keywords = re.sub(r'\d+%?', '', temperature_keywords).strip()  # 去除數字和百分比
                temperature_keywords = temperature_keywords.replace("推薦-", "").replace("推薦", "").strip()  # 去除"推薦"字樣
                print(f"🔍 冰熱選擇關鍵詞: {temperature_keywords}")
                ...
                for opt in all_options:
                    try:
                        ...
                        # 簡單模糊匹配: 只要選項包含關鍵詞，就選取它
                        if temperature_keywords in opt_text:
                            driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", opt)
                            time.sleep(1)
                            driver.execute_script("arguments[0].click();", opt)
                            print(f"✅ 已選擇冰熱選項: {opt_text} (匹配關鍵詞: {temperature_keywords})")
                            option_found = True
                            time.sleep(1)
                            break
                    except Exception as e:
                        print(f"處理選項時出錯: {str(e)}")
                        continue
                
                if not option_found:
                    print(f"⚠️ 未找到匹配 '{temperature_keywords}' 的冰熱選項")
            except Exception as e:
                print(f"❌ 處理冰熱選擇時出錯: {str(e)}")
        ...
        for option_key, option_value in food_options.items():
            ...
            try:
                print(f"🔍 處理選項: {option_key}: {option_value}")
                
                # 簡化的模糊匹配邏輯 - 提取關鍵詞
                option_keywords = re.sub(r'\s*\([^)]*\)', '', option_value).strip()  # 去除括號內容
                option_keywords = re.sub(r'【[^】]*】', '', option_keywords).strip()  # 去除【】內容
                option_keywords = re.sub(r'\d+%?', '', option_keywords).strip()  # 去除數字和百分比
                option_keywords = option_keywords.replace("推薦-", "").replace("推薦", "").strip()  # 去除"推薦"字樣
                print(f"🔍 選項關鍵詞: {option_keywords}")
                ...
                for opt in all_options:
                    try:
                        opt_text = opt.text.strip()
                        # 簡單模糊匹配: 只要選項包含關鍵詞，就選取它
                        if option_keywords in opt_text:
                            driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", opt)
                            time.sleep(1)
                            driver.execute_script("arguments[0].click();", opt)
                            print(f"✅ 已選擇選項: {opt_text} (匹配關鍵詞: {option_keywords})")
                            option_found = True
                            time.sleep(1)
                            break
                    except Exception as e:
                ...
                if not option_found:
                    print(f"⚠️ 未找到匹配 '{option_keywords}' 的選項")
            except Exception as e:
                print(f"❌ 處理選項 {option_key}: {option_value} 時出錯: {str(e)}")
